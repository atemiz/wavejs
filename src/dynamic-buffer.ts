export default class DynamicBuffer {
  ...
  private readonly _growSize = 4194304 // 4MB
  ...
  constructor(param?: ArrayBufferLike | number) {
  ...
  set offset(value: number) {
    this._offset = value
    if (value > this._length) this._length = value
  }
  ...
  ensureSize(size: number, offset = this._buffer.byteLength) {
    if (size + offset > this._buffer.byteLength) this.grow((size + offset) * 2)
  }

  write(buffer: ArrayBufferLike, offset = this._offset) {
  ...
  writeFourCC(value: string, offset = this._offset) {
  ...
  writeU16(value: number, offset = this._offset) {
  ...
  writeU32(value: number, offset = this._offset) {
    this.ensureSize(4, offset)
  ...
  writeF32(value: number, offset = this._offset) {
    this.ensureSize(4, offset)
    this._view.setFloat32(offset, value, true)
    this.offset = offset + 4
  }

  addF32(value: number, offset = this._offset) {
    this.ensureSize(4, offset)
